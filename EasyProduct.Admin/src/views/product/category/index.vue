        ...
        >
          <template #header>
            <div class="tree-header">
              <span>{{ t('product.category.tree.title') }}</span>
              <el-button
                type="primary"
                size="small"
                @click="handleAddRoot"
              >
                {{ t('product.category.tree.addRoot') }}
              </el-button>
            </div>
          </template>
          ...
          <el-input
            v-model="filterText"
            :placeholder="t('product.category.tree.searchPlaceholder')"
            clearable
            class="tree-search"
          />
            ...
            <template #default="{ data }">
              <span class="tree-node">
                ...
                <span class="tree-actions">
                  ...
                  >
                    {{ t('product.category.tree.addChild') }}
                  </el-button>
                  <el-button
                    link
                    size="small"
                  ...
                  >
                    {{ t('product.category.tree.delete') }}
                  </el-button>
                </span>
              </span>
            </template>
        ...
        >
          <template #header>
            <div class="detail-header">
              <span>{{ currentCategory?.name || t('product.category.detail.selectCategory') }}</span>
              <el-button
          ...
          <div
            ...
            class="empty-tip"
          >
            <el-empty :description="t('product.category.detail.selectCategoryTip')" />
          </div>
          ...
          >
            <el-descriptions
              ...
              border
            >
              <el-descriptions-item :label="t('product.category.detail.name')">
                {{ currentCategory.name }}
              </el-descriptions-item>
              <el-descriptions-item :label="t('product.category.detail.nameEn')">
                {{ currentCategory.nameEn || '-' }}
              </el-descriptions-item>
              <el-descriptions-item :label="t('product.category.detail.parent')">
                {{ getParentName(currentCategory.parentId) }}
              </el-descriptions-item>
              <el-descriptions-item :label="t('product.category.detail.sort')">
                {{ currentCategory.sort }}
              </el-descriptions-item>
              <el-descriptions-item :label="t('product.category.detail.status')">
                <BaseStatusTag
                  :value="currentCategory.status"
                  :options="ENABLED_DISABLED_STATUS"
                />
              </el-descriptions-item>
              <el-descriptions-item :label="t('product.category.detail.createdAt')">
          ...
          >
            <div class="section-header">
              {{ t('product.category.subcategory.title') }}
            </div>
            <el-table
              :data="subCategories"
              size="small"
            ...
            >
              <el-table-column
                prop="name"
                :label="t('product.category.detail.name')"
              />
              <el-table-column
                prop="nameEn"
                :label="t('product.category.detail.nameEn')"
              />
              <el-table-column
                prop="sort"
                :label="t('product.category.detail.sort')"
                width="80"
              />
              <el-table-column
                prop="status"
                :label="t('product.category.detail.status')"
                width="80"
              >
...
import { getCategoryTree, deleteCategory } from '@/api/product/category'
...
const getParentName = (parentId: string): string => {
  if (!parentId || parentId === '0') return t('product.category.rootName')
...
const handleDelete = async (data: ProductCategory): Promise<void> => {
  try {
    ...
    ElMessage.success(t('product.category.message.deleteSuccess'))
